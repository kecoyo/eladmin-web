<template>
  <div class="app-container">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-if="checkPer(['admin', 'grade_admin:del'])" label="操作" width="200px" align="center">
...
<script>
import { addGradeAdmin, deleteGradeAdmin } from '@/api/business/school'
...
const defaultForm = { gradeLevel: null, ids: [], selectedIds: [] }
export default {
  name: 'GradeAdminPane',
  ...
  cruds() {
    return CRUD({
      title: '年级管理员',
      url: 'ljadmin/school/queryGradeAdmin',
      delParams: {},
      crudMethod: {
        add: addGradeAdmin,
  ...
  data() {
    return {
      ...
      schoolGrades: [],
      permission: {
        add: ['admin', 'grade_admin:add'],
        del: ['admin', 'grade_admin:del']
      },
      rules: {
  ...
  mounted() {
    this.$eventBus.$on('schoolGradesChange', schoolGrades => {
      this.schoolGrades = schoolGrades
      this.gradeLevel = schoolGrades[0].gradeLevel
    })
  },
  beforeDestroy() {
    this.$eventBus.$off('schoolGradesChange')
  },
  methods: {
    [CRUD.HOOK.beforeRefresh](curd) {
      curd.query = { ...curd.query, ...this.query, schoolId: this.schoolId, gradeLevel: this.gradeLevel }
      return this.gradeLevel > 0
    },
    // 新增编辑前做的操作
    [CRUD.HOOK.beforeToCU](crud, form) {
      this.form.schoolId = this.schoolId
      this.form.gradeLevel = this.gradeLevel
    },
    // 提交前做的操作
    ...
    [CRUD.HOOK.beforeDelete](crud, data) {
      crud.delParams = { schoolId: this.schoolId, gradeLevel: this.gradeLevel }
      return true
    }
  }
}
