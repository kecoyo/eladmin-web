<template>
  <div class="pane-container">
    <el-descriptions v-if="studentInfo" class="margin-top" :column="1" size="medium" border label-class-name="descriptions-label">
      ...
      <el-descriptions-item>
        ...
        {{ GENDER.getName(studentInfo.gender) }}
      </el-descriptions-item>
      <el-descriptions-item>
        <template slot="label">
          手机号
      ...
      <el-descriptions-item>
        ...
        <el-image :src="studentInfo.faceImg || defaultFaceImg" :fit="'fit'" :preview-src-list="[studentInfo.faceImg || defaultFaceImg]" style="width: 100px; height: 100px" />
      </el-descriptions-item>
      <el-descriptions-item>
        <template slot="label">
          身份证号
    ...
    <div class="pane-footer">
      <el-button type="primary" size="small" icon="el-icon-edit" @click.stop="crud.toEdit(studentInfo)">修改</el-button>
    </div>
    <!--表单组件-->
...
<script>
import { updateStudent } from '@/api/business/student'
...
export default {
  ...
  components: {},
  cruds() {
    return CRUD({ title: '学生', url: 'ljadmin/student/getStudentInfo', crudMethod: { edit: updateStudent } })
  },
  mixins: [presenter(), form(defaultForm), crud()],
  data() {
    return {
      id: Number(this.$route.params.studentId),
      studentInfo: null,
  ...
  methods: {
    ...
    [CRUD.HOOK.afterRefresh](curd) {
      this.studentInfo = curd.data
      this.$eventBus.$emit('studentInfoChange', this.studentInfo)
